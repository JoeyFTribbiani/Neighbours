...
AV.Cloud.define("getPhoneAuthenticationCode",function(req, res) {
    ...
    AV.Query.doCloudQuery("select count(*) from PhoneAuthenticationCode where phone=? and createdAt>=? and createdAt<=?",[phone,new Date(moment(new Date()).format('yyyy-MM-dd')+'T00:00:00.000Z'),new Date(moment(new Date()).format('yyyy-MM-dd')+'T23:59:59:999Z')],{
